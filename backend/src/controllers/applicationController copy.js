...
// Fixed: getApplicationActivityLogs controller
      ...
      if (log.old_value) {
        try {
          const oldVal = typeof log.old_value === 'string' 
            ...
            : log.old_value;
          plantId = oldVal?.plant_location_id; // ✅ Use optional chaining
        } catch (e) {
          // Ignore parse errors
        }
      }
      ...
      if (!plantId && log.new_value) {
        try {
          const newVal = typeof log.new_value === 'string' 
            ...
            : log.new_value;
          plantId = newVal?.plant_location_id; // ✅ Use optional chaining
        } catch (e) {
          // Ignore parse errors
        }
      }
      
      // ✅ FIX: If no plant ID found, include the log (system-level logs)
      if (!plantId || plantId === null || plantId === undefined) {
        return true;
      }
      
      // ✅ FIX: Safely check if user can access this plant
      try {
        // Convert plantId to number to ensure valid type
        const numericPlantId = Number(plantId);
        
        // Skip invalid plant IDs
        if (isNaN(numericPlantId)) {
          return true; // Include logs with invalid plant IDs
        }
        
        return canAccessPlant(req.user, numericPlantId);
      } catch (error) {
        // ✅ FIX: If canAccessPlant throws an error, include the log
