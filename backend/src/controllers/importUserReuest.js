...
const ImportUserRequest: React.FC = () => {
  ...
  const fetchReferenceData = async () => {
    ...
    try {
      ...
      // Fetch all applications (with multiple_role_access flag)
      const appsRes = await fetch(`${API_BASE}/api/applications`, { headers });
      if (appsRes.ok) setAllApplications(await appsRes.json());

      // Fetch all app-role mappings (role_id, role_name, application_id)
      const appRolesRes = await fetch(`${API_BASE}/api/app-roles`, { headers });
      if (appRolesRes.ok) setAllAppRoles(await appRolesRes.json());

      // Fetch all vendors (for vendor_firm dropdown)
      const vendorsRes = await fetch(`${API_BASE}/api/vendors`, { headers });
      if (vendorsRes.ok) {
        const vendorData = await vendorsRes.json();
        // Keep only active vendors (mirror UserRequestForm logic)
        const activeVendors = Array.isArray(vendorData) ? vendorData.filter((v: any) => {
          if (v.active !== undefined) return v.active === true;
          if (v.status !== undefined) return ["ACTIVE","Active","active"].includes(v.status);
          return true;
        }) : [];
  ...
  const downloadTemplate = async () => {
    try {
      ...
      ws.addRow({
        ...
        application_equip_id:       sampleApp   ? `${sampleApp.id} - ${sampleApp.display_name || sampleApp.name}` : "1 - Application Name",
      ...
      ws.addRow({
        ...
        vendor_firm:                sampleVendor ? sampleVendor.vendor_name : "Vendor Firm Ltd",
        vendor_code:                sampleVendor ? sampleVendor.vendor_code : "VND001",
      ...
      // Every select field from UserRequestForm gets a dropdown here.
      // Lookup Data column map:
      //   A = AccessRequestType   B = PlantOptions     C = DeptOptions
      //   D = ApplicationOptions  E = RoleOptions      F = RequestForBy
      //   G = UserRequestType     H = TrainingStatus   I = VendorFirmOptions
      const maxRow = 2000;
      for (let i = 4; i <= maxRow; i++) {
        // Col A — request_for_by  (Self / Others / Vendor / OEM)
        ws.getCell(`A${i}`).dataValidation = {
          type: "list", allowBlank: false,
          formulae: [`'Lookup Data'!$F$2:$F$${REQUEST_FOR_OPTIONS.length + 1}`],
          showErrorMessage: true,
          errorTitle: "Invalid",
          error: "Select: Self, Others, or Vendor / OEM",
        };
        // Col E — access_request_type
        ws.getCell(`E${i}`).dataValidation = {
          type: "list", allowBlank: false,
          formulae: [`'Lookup Data'!$A$2:$A$${ACCESS_TYPES_ALL.length + 1}`],
          showErrorMessage: true,
          errorTitle: "Invalid Access Type",
          error: "Select a valid access request type from the list",
        };
        // Col F — training_status  (Yes / No)
        ws.getCell(`F${i}`).dataValidation = {
          type: "list", allowBlank: false,
          formulae: [`'Lookup Data'!$H$2:$H$${TRAINING_OPTIONS.length + 1}`],
          showErrorMessage: true,
          errorTitle: "Invalid",
          error: "Select Yes or No",
        };
        // Col G — user_request_type  (Permanent / Temporary)
        ws.getCell(`G${i}`).dataValidation = {
          type: "list", allowBlank: true,
          formulae: [`'Lookup Data'!$G$2:$G$${USER_REQUEST_TYPE_OPTIONS.length + 1}`],
          showErrorMessage: true,
          errorTitle: "Invalid",
          error: "Select Permanent or Temporary",
        };
        // Col P — vendor_firm  (Vendor / OEM rows only — leave blank for Self/Others)
        // vendor_name (O) is a free-text person name; vendor_firm (P) is the company dropdown
        if (allVendors.length > 0) {
          ws.getCell(`P${i}`).dataValidation = {
            type: "list", allowBlank: true,
            formulae: [`'Lookup Data'!$I$2:$I$${allVendors.length + 1}`],
            showErrorMessage: true,
            errorTitle: "Invalid Vendor Firm",
            error: "Select a vendor firm from the Lookup Data sheet (col I)",
          };
        }
        // Col S — plant_location_id
        if (plants.length > 0) {
          ws.getCell(`S${i}`).dataValidation = {
            ...
            error: "Select a plant from the Lookup Data sheet (col B)",
          };
        }
        // Col T — department_id
        if (allDepartments.length > 0) {
          ws.getCell(`T${i}`).dataValidation = {
            ...
            error: "Select a department from the Lookup Data sheet (col C)",
          };
        }
        // Col U — application_equip_id  (apps marked "(multi-role)" allow 2 roles)
        if (allApplications.length > 0) {
          ws.getCell(`U${i}`).dataValidation = {
            type: "list", allowBlank: true,
            formulae: [`'Lookup Data'!$D$2:$D$${allApplications.length + 1}`],
            showErrorMessage: true,
            errorTitle: "Invalid Application",
            error: "Select an application from the Lookup Data sheet (col D)",
          };
        }
        // Col V — role_id
        // For multi-role apps type: "ID - Name, ID - Name" (max 2). showErrorMessage:false
        // so Excel allows comma-separated free text. Validated in validateRecord.
        if (allRoles.length > 0) {
          ws.getCell(`V${i}`).dataValidation = {
            type: "list", allowBlank: true,
            formulae: [`'Lookup Data'!$E$2:$E$${allRoles.length + 1}`],
      ...
      // ── Sheet 2: Lookup Data ─────────────────────────────────────────────
      // Column layout (matches formulae references above):
      //   A=AccessRequestType  B=PlantOptions  C=DeptOptions  D=ApplicationOptions
      //   E=RoleOptions  F=RequestForBy  G=UserRequestType  H=TrainingStatus  I=VendorFirmOptions
      const lookupSheet = workbook.addWorksheet("Lookup Data");
      lookupSheet.columns = [
        { header: "AccessRequestType",  key: "access_type", width: 40 }, // A
        { header: "PlantOptions",       key: "plants",      width: 44 }, // B
        { header: "DeptOptions",        key: "depts",       width: 44 }, // C
        { header: "ApplicationOptions", key: "apps",        width: 48 }, // D — "(multi-role)" tagged
        { header: "RoleOptions",        key: "roles",       width: 36 }, // E
        { header: "RequestForBy",       key: "req_for",     width: 18 }, // F
        { header: "UserRequestType",    key: "ur_type",     width: 18 }, // G
        { header: "TrainingStatus",     key: "training",    width: 16 }, // H
        { header: "VendorFirmOptions",  key: "vendors",     width: 36 }, // I
      ];
      ...
      for (let i = 0; i < maxLookup; i++) {
        ...
        const appLabel = app
          ? `${app.id} - ${app.display_name || app.name}${app.multiple_role_access ? " (multi-role)" : ""}`
          : "";
        ...
        lookupSheet.addRow({
          ...
          vendors:     vendor                   ? vendor.vendor_name : "",
        });
      }

      // Style lookup data rows alternating
      for (let r = 2; r <= maxLookup + 1; r++) {
        lookupSheet.getRow(r).eachCell((cell) => {
      ...
      lookupSheet.getCell("D1").note = {
        texts: [{ text: 'Apps marked "(multi-role)" allow up to 2 roles (comma-separated) in role_id. Others allow only 1.' }],
      };
      lookupSheet.getCell("E1").note = {
        texts: [{ text: 'For multi-role apps: enter up to 2 roles as "ID - Name, ID - Name" in the role_id column.' }],
      };
      lookupSheet.getCell("I1").note = {
        texts: [{ text: 'Vendor / OEM rows only. Leave blank for Self or Others.' }],
      };
      ...
      const guide = [
        // col  req    description                                                              example
        ["Col A  request_for_by",             "YES",  "DROPDOWN ▾ — Who the request is for (Lookup Data col F)",              "Self"],
        ["Col B  name",                       "YES",  "Full name of the person access is for (free text)",                    "John Doe"],
        ["Col C  employee_code",              "YES*", "Employee code — *not required for Vendor / OEM (free text)",           "EMP001"],
        ["Col D  employee_location",          "NO",   "City / location of the employee (free text)",                          "Mumbai"],
        ["Col E  access_request_type",        "YES",  "DROPDOWN ▾ — Access type (Lookup Data col A)",                        "New User Creation"],
        ["Col F  training_status",            "YES",  "DROPDOWN ▾ — Training completed? (Lookup Data col H)",                "Yes"],
        ["Col G  user_request_type",          "NO",   "DROPDOWN ▾ — Permanent or Temporary (Lookup Data col G)",             "Permanent"],
        ["Col H  from_date",                  "NO",   "Start date for Temporary access YYYY-MM-DD (free text)",               "2024-01-01"],
        ["Col I  to_date",                    "NO",   "End date for Temporary access YYYY-MM-DD — required if Temporary",     "2024-12-31"],
        ["Col J  approver1_email",            "YES",  "Email of Approver 1 / Manager (free text)",                           "mgr@company.com"],
        ["Col K  approver2_email",            "NO",   "Email of Approver 2 — optional (free text)",                          "dir@company.com"],
        ["Col L  remarks",                    "NO",   "Any notes or reason for the request (free text)",                      "New joiner"],
        ["Col M  request_raised_by",          "NO",   "Name of person raising the request (free text)",                       "HR Admin"],
        ["Col N  request_raised_by_emp_code", "NO",   "Employee code of person raising the request (free text)",             "ADMIN01"],
        ["Col O  vendor_name",                "V",    "Vendor person name — Vendor / OEM only (free text)",                  "Vendor Contact"],
        ["Col P  vendor_firm",                "V",    "DROPDOWN ▾ — Vendor firm (Lookup Data col I) — Vendor / OEM only",   "Vendor Firm Ltd"],
        ["Col Q  vendor_code",                "V",    "Vendor code — auto-filled from vendor_firm selection (free text)",    "VND001"],
        ["Col R  vendor_allocated_id",        "V",    "Allocated ID for vendor — Vendor / OEM only (free text)",             "VALOC001"],
        ["Col S  plant_location_id",          "YES",  "DROPDOWN ▾ — Plant (Lookup Data col B) — format: ID - Name",         "2 - Mumbai Plant"],
        ["Col T  department_id",              "YES",  "DROPDOWN ▾ — Department (Lookup Data col C) — format: ID - Name",    "3 - IT Dept"],
        ["Col U  application_equip_id",       "YES*", "DROPDOWN ▾ — Application (Lookup Data col D). Apps marked (multi-role) allow 2 roles.", "5 - SAP ERP"],
        ["Col V  role_id",                    "YES*", "DROPDOWN ▾ — Role (Lookup Data col E). Multi-role app: type 2 as 'ID - Name, ID - Name'", "7 - Admin Role"],
        ["Col W  reports_to",                 "NO",   "Manager name the user reports to (free text)",                        "Jane Smith"],
        ["Col X  assignment_group",           "NO",   "IT group for the task — import use only (free text)",                 "IT-GRP-MUM"],
        ["Col Y  allocated_id",               "NO",   "System ID assigned to user — defaults to employee_code (free text)", "EMP001"],
        ["Col Z  role_granted",               "NO",   "Exact role granted — defaults to role name (free text)",              "Admin Role"],
      ];

      guide.forEach(([col, req, desc, example]) => {
        const row = guideSheet.addRow({ col, req, desc, example });
        // Required column colour
        if (req === "YES") {
          row.getCell("req").font = { bold: true, color: { argb: "FFB91C1C" } };
        } else if (req === "YES*") {
          row.getCell("req").font = { bold: true, color: { argb: "FFD97706" } };
        } else if (req === "V") {
          row.getCell("req").font = { bold: true, color: { argb: "FF7C3AED" } };
        }
        // Highlight rows that have a dropdown with a light green tint
        if (String(desc).includes("DROPDOWN")) {
          row.eachCell((cell) => {
            cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE8F5E9" } };
          });
        }
      });
      guideSheet.getColumn("col").alignment  = { vertical: "middle" };
      guideSheet.getColumn("req").alignment  = { horizontal: "center", vertical: "middle" };
      guideSheet.getColumn("desc").alignment = { wrapText: true, vertical: "middle" };

      // Legend row at the bottom
      const legendRow = guideSheet.addRow({ col: "Legend:", req: "", desc: "GREEN rows = Excel dropdown available  |  YES* = conditional required  |  V = Vendor/OEM only", example: "" });
  ...
  const validateRecord = (data: any, rowNumber: number): ImportRecord => {
    ...
    // ── role_id — required for role-enabled types (max 2; single if !multiple_role_access) ─
    if (!isBulkDeactivation && isRoleEnabled) {
      if (roleIds.length === 0) {
        errors.push({ row: rowNumber, field: "role_id", message: "role_id is required for this access type", value: data.role_id });
      } else {
        // Look up the application to check multiple_role_access flag
        const appId = data.application_equip_id ? Number(data.application_equip_id) : null;
        const matchedApp = appId ? allApplications.find((a) => a.id === appId) : null;
        const isMultiRoleAllowed = matchedApp?.multiple_role_access === true;

        if (isMultiRoleAllowed) {
          // multiple_role_access apps: allow up to 2 roles
          if (roleIds.length > 2) {
            errors.push({
              row: rowNumber,
              field: "role_id",
              message: `This application allows multiple roles — maximum 2 roles allowed per task row. Found ${roleIds.length}.`,
              value: data.role_id,
            });
          }
        } else {
          // Single-role apps: only 1 role allowed
          if (roleIds.length > 1) {
            errors.push({
              row: rowNumber,
              field: "role_id",
              message: `This application does not allow multiple roles. Only 1 role can be selected. Found ${roleIds.length}.`,
              value: data.role_id,
            });
          }
        }
