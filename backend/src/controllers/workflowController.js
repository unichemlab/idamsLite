...
const hasSpecificFilter = transaction_id || plant_id || plant || approver_id;

if (isApprover && !hasSpecificFilter) {
  params.push(`%,${user_id},%`);
  const p1 = `$${params.length}`;

  where.push(`(
    CONCAT(',', approver_1_id, ',') LIKE ${p1} OR
    CONCAT(',', approver_2_id, ',') LIKE ${p1} OR
    CONCAT(',', approver_3_id, ',') LIKE ${p1} OR
    CONCAT(',', approver_4_id, ',') LIKE ${p1} OR
    CONCAT(',', approver_5_id, ',') LIKE ${p1}
  )`);
}
