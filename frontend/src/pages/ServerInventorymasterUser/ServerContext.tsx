...
interface ServerContextType {
  servers: Server[];
  refreshServers: () => void;
  addServer:    (server: Server)              => Promise<ApprovalResponse | Server>;
  updateServer: (id: number, updated: Server) => Promise<ApprovalResponse | Server>;
...
export const ServerProvider = ({ children }: { children: ReactNode }) => {
  ...
  // ── Add Server ──────────────────────────────────────────────────────────────
  const addServer = async (server: Server): Promise<ApprovalResponse | Server> => {
    try {
      const { id, created_on, updated_on, transaction_id, ...payload } = server;
      normalizePayload(payload);

      const response = await addServerAPI(payload);
  ...
  const updateServer = async (
    id: number,
    updated: Server
  ): Promise<ApprovalResponse | Server> => {
    try {
      ...
      const response = await updateServerAPI(id, payload);
