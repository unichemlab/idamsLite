...
const ImportPlant: React.FC = () => {
  ...
  const requiredFields = ['plant_name','location', 'description', 'status'];
  ...
  const downloadTemplate = () => {
    const headers = ['plant_name','location', 'description', 'status'];
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.aoa_to_sheet([headers, ['	Goa Plant 1','Goa, India', 'Goa Plant 1 facility for pharmaceutical manufacturing unit', 'ACTIVE']]);
    XLSX.utils.book_append_sheet(wb, ws, 'Plant Template');
    XLSX.writeFile(wb, 'plant_import_template.xlsx');
  };
 ...
 const handleValidate = async () => {
  ...
  try {
    ...
    jsonData.forEach((row: any, index: number) => {
      const plant = row.plant_name?.toLowerCase()?.trim();
      if (!plant) return;

      if (!plantMap[plant]) plantMap[plant] = [];
      plantMap[plant].push(index + 2); // Excel row number
    });
    ...
    const plantNames = jsonData
      .map((r: any) => r.plant_name?.toLowerCase()?.trim())
      .filter((r: string) => r);
    ...
    const validatedRecords: ImportRecord[] = jsonData.map((row: any, index: number) => {
      ...
      // ❌ Excel duplicate
      if (plant && plantMap[plant]?.length > 1) {
        record.errors.push({
          row: index + 2,
          field: 'plant_name',
          message: `Duplicate in file (rows: ${plantMap[plant].join(', ')})`,
      ...
      // ❌ DB duplicate
      if (plant && dbDuplicateMap[plant]) {
        record.errors.push({
          row: index + 2,
          field: 'plant_name',
          message: `Plant already exists in system`,
          value: row.plant_name
        });
        record.isValid = false;
      }
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/plants/import`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  return (
    <div className={styles.pageWrapper}>
      <AppHeader title="Import Plants" />
      
      <div className={styles.contentArea}>
        <div className={styles.formCard}>
          <div className={styles.formHeader}>
            <h2>Import Plants from Excel</h2>
            <p>Upload an Excel file to bulk import Plant records</p>
          </div>
          ...
          <div style={{ padding: '20px' }}>
            ...
            <div style={{ marginTop: '30px' }}>
              <button className={styles.cancelBtn} onClick={() => navigate('/plant-master')}>
                Cancel
              </button>
            </div>
          </div>
